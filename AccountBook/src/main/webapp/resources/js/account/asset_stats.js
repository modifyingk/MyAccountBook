...
$(function() {
	...
	$(document).ready(function() {
		...
		// 자산 통계
		$.showStats($.assetStatsList(today, userid, "spend"));
	...
	$(document).on("click", "#in-stats-btn", function() {
		...
		$.showStats($.assetStatsList(today, userid, "income"));
	...
	$(document).on("click", ".tr-statsName", function() {
		var assetname = $(this).children().eq(0).text();
		var moneytype;
		...
		var html = $.accountHtml($.detailsOfAsset(assetname, moneytype, today, userid));
		$("#account-list-div").html(html);
		$("#name-div").html("<i class='fi fi-rr-usd-circle'></i> " + assetname); // 자산명 html
		$.showStatsTotal(moneytype); // 자산별 합계 html
		$("#details-div").show();
	})
