			...
			success : function(file) {
				if(file != null) {
					...
					for(var i = 0; i < file.length; i++) {
						var fileCallPath = encodeURIComponent(file[i].uploadpath + "\\s_" + file[i].uuid + "_" + file[i].filename);
						
						str += "<li data-path='" + file[i].uploadpath + "' data-uuid='" + file[i].uuid + "' data-filename='" + file[i].filename + "' data-type='image'>";
						str += "<span>" + file[i].filename + "</span><button type='button' data-file=\'" + fileCallPath + "\' data-type='image'>x</button>" +
								"<img id='upload-files' src='display?filename=" + fileCallPath + "'></li>";
					}
					$("#upload-list-div ul").append(str);
				}
